...
Yellfy построен на [Normalize.css](http://necolas.github.io/normalize.css/) и включает в себя микро CSS-фреймворк, включающий в себя лишь всё самое необходимое для быстрого старта работы над практически любым проектом: типографику, сетку, кнопки, формы, списки и таблицы.
...
Для быстрого старта необходимо выполнить лишь одну команду, которая запустит первичную сборку проекта и слежение за изменением файлов:

```shell
$ gulp
```

Чтобы получить полный список доступных команд в консоли, используйте стандартную Gulp-команду:
...
Запускает первичный цикл сборки проекта и запускает сервер на `8000` порту.
...
Оптимизация файлов стилей и скриптов, а также оптимизация изображений. Также доступны отдельные задачи для каждого типа файлов:
...
Сборка файлов тестов и запуск сервера на `8001` порту.
