...
Кроме продуманной системы задач Yellfy предлагает свой вариант организации файловой системы проектов:

**Директория исходных файлов (app)**

  * `fonts/`       — шрифты
  * `images/`      — изображения
    * `icons/`     — иконки, из которых формируется SVG-спрайт
    * `touch/`     — иконки для соц. сетей и браузеров
  * `scripts`      — скрипты
    * `modules/`   — файлы модулей
    * `tests/`     — файлы тестов по типу `модуль.spec.js`
    * `vendor/`    — вендорские файлы скриптов
    * `scripts.js` — точка входа скриптов
    * `tests.js`   — точка входа тестов
  * `styles/`      — стили
    * `less/`      — файлы стилей
    * `vendor/`    — вендорские файлы стилей
  * `templates/`   — шаблоны
    * `index.pug`  — индексовая страница

**Служебная директория (gulp)**

  * `config/`    — конфигурационные файлы линтеров
  * `helpers/`   — помощники для задач
  * `tasks/`     — задачи
  * `templates/` — шаблоны, используемые `yellfy-cli` при генерации страниц и компонентов

Подробное описание организации файловой структуры стилей и шаблонов, смотрите, соответственно, в статье [«Организация кода для CSS-препроцессора»](https://canonium.com/articles/organizing-code-in-project) и [«Организация кода для HTML-препроцессоров»](https://canonium.com/articles/organizing-html-code-in-project).
...
В случае, если вы хотите использовать нестандартный CSS- или HTML-препроцессор вместе с консольной утилитой, вам необходимо изменить настройки Yellfy.

### Настройки шаблонов

Шаблоны, используемые при генерации страниц и компонентов из консольной утилиты, находятся в директории `gulp/templates`. Вы можете изменить директорию, используя поле `yellfy.path` в файле `package.json`. Также пользователю доступны настройки текущего CSS- и HTML-препроцессора.

Например, если вы собираетесь использовать Nunjucks в качестве HTML-препроцессора, необходимо создать файлы `page.index.njk.tpl` и `page.markup.njk.tpl`, а затем указать расширение `njk` в поле `yellfy.html`
